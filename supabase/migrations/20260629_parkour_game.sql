-- Zoo-Parkour Minispiel: 12 Level, Fortschritt + Sterne pro Level.
-- Erstabschluss zahlt volle Belohnung (Coins, alle 3 Level Tickets),
-- Wiederholungen nur einen kleinen Coin-Bonus.
-- Spiegelt das Drift-Belohnungssystem, damit die Wirtschaft balance-neutral bleibt.

create table if not exists public.parkour_progress (
...
alter table public.parkour_progress enable row level security;
drop policy if exists "parkour_progress self read" on public.parkour_progress;
create policy "parkour_progress self read" on public.parkour_progress
  for select using ((select auth.uid()) = user_id);

revoke all on table public.parkour_progress from anon;
grant select on table public.parkour_progress to authenticated;

-- Belohnung pro Level: Coins wachsen quadratisch, Tickets auf Level 3/6/9/12.
create or replace function public._parkour_reward(p_level int)
...
create or replace function public.get_parkour_progress()
returns jsonb language plpgsql security definer set search_path = public as $$
declare
  uid uuid := auth.uid();
  v_row public.parkour_progress%rowtype;
begin
  if uid is null then raise exception 'not authenticated'; end if;
  select * into v_row from public.parkour_progress where user_id = uid;
...
grant execute on function public.get_parkour_progress() to authenticated;
...
revoke execute on function public.get_parkour_progress() from anon, public;
