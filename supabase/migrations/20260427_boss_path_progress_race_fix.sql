-- Boss-Pfad: Race-Bedingung in complete_boss_stage beheben + bestehende inkonsistente Fortschritte heilen.
-- Problem: zwei parallele Aufrufe von complete_boss_stage mit gleicher current_stage
-- konnten beide die Pruefung passieren, beide Belohnungen einfuegen und beide
-- total_victories hochzaehlen. Folge: doppelte Belohnungen und total_victories,
-- und in Kombination mit historischen Resets ein current_stage < highest_stage.

-- 1) Bestehende inkonsistenz heilen: aktuelle Stufe darf nie hinter der hoechsten liegen.
update public.boss_path_progress
   set current_stage = highest_stage + 1,
       updated_at = now()
 where current_stage <= highest_stage;

-- 2) complete_boss_stage neu definieren mit Row-Lock auf der Fortschritts-Zeile,
--    damit parallele Aufrufe serialisiert werden.
...
begin
  ...
  -- Row-Lock: serialisiert konkurrierende Aufrufe fuer diesen User.
  select * into prog from public.boss_path_progress
   where user_id = uid
   for update;
