...
watch(() => auth.isAuth, (v) => { if (v) subscribeBroadcasts(); else if (broadcastChannel) { supabase.removeChannel(broadcastChannel); broadcastChannel = null } })
  ...
  <div class="app-shell">
    <header v-if="showNav" class="top-bar">
      ...
      <div class="top-right">
        ...
        <router-link to="/settings" class="settings-link" title="Einstellungen">⚙️</router-link>
      </div>
    </header>
...
.broadcast-icon { font-size: 32px; }
.broadcast-text { white-space: pre-wrap; word-wrap: break-word; }
.broadcast-fade-enter-active, .broadcast-fade-leave-active { transition: opacity 0.3s, transform 0.3s; }
.broadcast-fade-enter-from { opacity: 0; transform: translate(-50%, -60%) scale(0.9); }
.broadcast-fade-leave-to { opacity: 0; transform: translate(-50%, -40%) scale(0.95); }
</style>
